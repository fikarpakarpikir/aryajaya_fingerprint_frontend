...
function FaceRecog() {
    ...
    async function fetchData() {
        ...
        // convert to plain HTMLImageElements array for convenience (or grouped per label)
            ...
            const labeled = await Promise.all(
                labels.map(async (label) => {
                    ...
                    for (let i = 0; i < dataFaceID.length; i++) {
                        setSubMessage(
                            `Memindai gambar ${i + 1} untuk ${label}`
                        );
    ...
    const createFormRegist = (foto, idFace, kar) => {
